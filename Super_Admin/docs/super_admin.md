...
## Signup
- `POST /signup`
- Body: `{ "email": "user@example.com", "password": "secret", "name": "Required Name" }`
- Success: `201 Created` with `superAdmin` summary (no token is issued on signup).

## Login
- `POST /login`
- Body: `{ "email": "user@example.com", "password": "secret" }`
- Success: `200 OK` with new `token` and `superAdmin` summary.
...
- Body (optional): `{ "approverEmail": "superadmin@example.com" }`
- Success: `200 OK` with updated vendor marked as `approved` by super admin.

## Reject Vendor
- `POST /vendors/:id/reject`
- Body (optional): `{ "approverEmail": "superadmin@example.com", "reason": "Optional reason" }`
- Success: `200 OK` with updated vendor marked as `rejected` by super admin.

## Forgot Password
- `POST /api/auth/forgot-password`
- Body: `{ "email": "<super admin email>" }`
...
**Example:**
```bash
curl -X GET "http://localhost:3000/api/super-admin/products?page=1&limit=10&isActive=true" \
...
- Passwords are stored using salted PBKDF2 hashing (no plaintext passwords).
